...
def shuffle_array(nums, n):
    ...
    for i in range(n):
        result.append(first_half[i])   # Add element from first half
        result.append(second_half[i])  # Add element from second half
...
def count_characters(s):
    ...
    for char in s:
        if char in counts:
            counts[char] += 1  # Increment if already exists
        else:
            counts[char] = 1   # Initialize to 1 if new
    return counts
